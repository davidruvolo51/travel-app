...
function summaryBoxes(id, data) {
  var parent = (0, _d.select)(id);
  var svg = parent.append("svg"); // update svg

  svg.attr("width", "100%").attr("height", "100%").attr("viewBox", "0 0 ".concat(boxDefs.chartWidth, " ").concat(boxDefs.chartHeight)).attr("preserveAspectRation", "xMinYmin").attr("class", "summary-boxes"); // render <g>

  var g = svg.selectAll("g").data(data).enter().append("g").attr("id", function (d) {
    return "box-".concat(d.labs.toLowerCase());
  }).attr("class", "summary-box"); // render <rect> in <g>

  var colCounter = 1;
  var rowCounter = 1;
  var rects = g.append("rect").attr("id", function (d) {
    return "box-value-".concat(d.labs.toLowerCase());
  }).attr("class", "bubble").attr("x", function (d, i) {
    if (i % boxDefs.rowLimit == 0) {
      colCounter = 0;
    } else {
      colCounter++;
    }

    return boxDefs.chartWidth / boxDefs.rowLimit * colCounter;
  }) // .attr("y", `${(boxDefs.chartHeight / data.length) * rowCounter}`)
  .attr("y", function (d, i) {
    if (i / colCounter == 0) {
      rowCounter++;
    } else {
    ...
    return boxDefs.chartHeight / data.length * rowCounter;
  }).attr("height", "".concat(boxDefs.chartHeight / boxDefs.rowLimit)).attr("width", "".concat(boxDefs.chartWidth / boxDefs.rowLimit)).attr("fill", boxDefs.fill); // render <text> on <rect>: values
  // let colCounter = 0;
  // let rowCounter = 1;
  // const values = g.append("text")
  //     .attr("class", "box-value")
  //     .attr("id", d => `box-value-${d.labs.toLowerCase()}`)
  //     .attr("x", (d, i) => {
  //          if (i <= boxDefs.rowLimit) {
  //              colCounter = colCounter + 1;
  //          } else {
  //              colCounter = 0;
  //              rowCounter = rowCounter + 1
  //         }
  //         return ((boxDefs.chartWidth / data.length) * colCounter + (boxDefs.chartWidth / data.length) * 0.5)
  //     })
  //     //  `${(boxDefs.chartWidth / data.length) * i + (boxDefs.chartWidth / data.length) * 0.5}`)
  //     .attr("y", `${(boxDefs.chartHeight / data.length) * 0.5}`)
  //     .text(d => d.vals);
  // render <text> on <rect>: labels
  // const labels = g.append("text")
  // .attr("class", "box-label")
  // .attr("id", d => `box-label-${d.labs.toLowerCase()}`)
  // .attr("x", (d, i) => `${(boxDefs.chartWidth / data.length) * i + (boxDefs.chartWidth / data.length) * 0.5}`)
  // .attr("y", `${((boxDefs.chartHeight / data.length) * 0.5) + (boxDefs.chartHeight / data.length) * 0.3}`)
  // .text(d => d.labs);
}
...
var _summaryBoxes = require("./viz/summaryBoxes");
...
// BEGIN
// imports
Shiny.addCustomMessageHandler("summaryBoxes", function (value) {
  setTimeout(function () {
    (0, _summaryBoxes.summaryBoxes)(value[0], value[1]);
  }, 150);
}); ////////////////////////////////////////
...
},{"./modules/shiny_handlers":"modules/shiny_handlers.js","./viz/summaryBoxes":"viz/summaryBoxes.js"}]},{},["index.js"], null)